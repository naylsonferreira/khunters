...
from .serializers import JogadorSerializer, Objeto_erSerializer, Objeto_er_mapSerializer, CapturaSerializer, MochilaSerializer
from website_app.models import Jogador, Objeto_er, Objeto_er_map, Captura, Mochila, Personagem
import time
...
def criar_automaticamente(latitude, longitude):
    ...
    for obj_er in lista:
        ...
        Objeto_er_map.objects.get_or_create(
            objeto_er_id=obj_er.id,
            latitude=latitude,
            longitude=longitude)
...
def personagens_proximos(request):
    ...
    for obj_er_map in Objeto_er_map.objects.all():
        try:
            localizacao_personagem = (
                obj_er_map.latitude, obj_er_map.longitude)
            distancia = 1000 * \
                distance(localizacao_player, localizacao_personagem).km
            ...
            pass
    for jogador_map in Jogador.objects.filter(
            online=True).exclude(pk=jogador.pk):
        try:
            localizacao_jogador = (jogador_map.latitude, jogador_map.longitude)
            distancia = 1000 * \
                distance(localizacao_player, localizacao_jogador).km
    ...
    if len(personagens) == 0:
        criar_automaticamente(latitude, longitude)
    for item in Mochila.objects.all().filter(user=user):
        mochila.append({"prefab": item.personagem.prefab,
...
def me(request):
    ...
    return JsonResponse(JogadorSerializer(
        jogador, many=False).data, safe=False)
...
def mochila(request, format=None):
    ...
    mochila = MochilaSerializer(
        Mochila.objects.all().filter(
            user=user), many=True).data
